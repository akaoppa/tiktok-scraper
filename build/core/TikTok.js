...
class TikTokScraper extends events_1.EventEmitter {
        ...
        for (let i = 0; i < posts.length; i += 1) {
            ...
            if (this.noDuplicates.indexOf(posts[i].id) === -1) {
                ...
                const item = Object.assign(Object.assign({ id: posts[i].id, secretID: posts[i].video.id, text: posts[i].desc, createTime: posts[i].createTime, authorMeta: {
                        id: posts[i].authorId,
                        secUid: posts[i].authorSecId,
                        name: posts[i].author,
                        nickName: posts[i].nickname,
                        verified: posts[i].officalItem,
                        signature: posts[i].signature,
                        avatar: posts[i].avatarLarger,
                    ...
                    }, webVideoUrl: `https://www.tiktok.com/@${posts[i].author}/video/${posts[i].id}`, videoUrl: posts[i].video.downloadAddr, videoUrlNoWaterMark: '', videoApiUrlNoWaterMark: '', videoMeta: {
    ...
    async getUserProfileInfo() {
        ...
        try {
            const response = await this.request(options);
            if (!response) {
                throw new Error(`Can't extract user meta data`);
            }
    ...
    async getVideoMeta(html = true) {
        ...
        const videoItem = {
            ...
            authorMeta: {
                ...
                verified: videoData.officalItem,
                private: videoData.secret,
                signature: videoData.signature,
                avatar: videoData.avatarLarger,
            },
            musicMeta: {
